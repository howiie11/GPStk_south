...
namespace gpstk
{
      ...
      throw(exception, FFStreamError, StringException)
   {
      ...
      SP3SatID sat;
			...
			// Line 3
			// The num of sats indicates the lines wo will loop
		int numSatsLines(5);
      strm.formattedGetLine(line);
		rows++;
      if(debug) std::cout << "SP3 Header Line 3"  << " " << line << std::endl;
      ...
      if (line[0]=='+')
      {
         ...
         svsAsWritten.resize(numSVs);
				...
				// This is case for some version c files
				// Mainly for version d 
			if( numSVs >= 85 )
			{
				numSatsLines = std::ceil( numSVs*1.0/17 );	// 17 sats per line

				if( (numSVs % 17) == 0 )								// Strange case
				{
					numSatsLines += 1;								
				}
			}

				// Store the satID in line 3
      ...
      {
         FFStreamError e("Unknown 1st char in line 3: "
            + string(1, line[0]));
         GPSTK_THROW(e);
      }

			// Read the rest lines containing SatID
			// lines 4:3 + (numSatsLines - 1)
		for( i = 4; i <= 3 + (numSatsLines - 1); i++ )					
		{
         strm.formattedGetLine(line);
			rows++;
		...
		}	// End of 'for( i = 4; i <= (numSatsLines - 1); i++ )'


         // read in the accuracy.
			// lines 3 + (numSatsLines) : 3 + 2*numSatsLines - 1
      readSVs = 0;
      for(i = 3+numSatsLines; i <= 3+2*numSatsLines-1; i++)
		{
			strm.formattedGetLine(line);
			rows++;
         if(debug) std::cout << "SP3 Header Line " << i << " " << line << std::endl;
		...
		} // End of ' for(i = 3+numSatsLines; i <= 3+2*numSatsLines-1; i++) '

			// The next two lines, with "%c" start
		strm.formattedGetLine(line);
		rows++;
      if(debug) std::cout << "SP3 Header Line " + asString(rows)+ " " << line << std::endl;
      if (version == SP3c) {
         if(line[0]=='%' && line[1]=='c')                         
         {
            // file system
         ...
         {
            FFStreamError e("Unknown label in line " + asString(rows) +": "+ line.substr(0,2));
            GPSTK_THROW(e);
         }
      }
      ...
      if(debug) std::cout << "SP3 Header Line " + asString(rows) + " " << line << std::endl;
			...
			// The next two lines, starting with %f 
		strm.formattedGetLine(line);
		rows++;
      if(debug) std::cout << "SP3 Header Line " + asString(rows) + " " << line << std::endl;
      if (version == SP3c) {
      ...
      for(i = 3+2*numSatsLines+4; i <= 3+2*numSatsLines+5; i++) {
         strm.formattedGetLine(line);
			rows++;
         if(debug) std::cout << "SP3 Header Line " << rows << " " << line << std::endl;
      }

			// read in comment lines
      comments.clear();

//      for(i = 1; i <= numComLines; i++)                           // lines 19-22
		while(1)
      {
			strm.formattedGetLine(line);
			rows++;
		...
		} // End of ' for(i = 1; i <= numComLines; i++) '

		if(debug)
		{
			std::cout << "SP3 1st Data Line " << rows << " " << line << std::endl;
		}
