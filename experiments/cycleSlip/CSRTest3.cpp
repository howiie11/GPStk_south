                           ...
                           int   precision )
{
         ...
         it++ )
   {
      ...
      for( typeValueMap::const_iterator itObs  = (*it).second.begin();
           ...
           itObs++ )
      {
		//	if( (*itObs).first == TypeID::satPCenter )
		//	{	
...
//      pObsFilter.setFilteredType(TypeID::P2);
      ...
      pList.push_back(elevWeights);       // Add to processing list
		...
		CodeBlunderDetection codeBlunderDetect;

		TypeIDSet codeTypes;
		codeTypes.insert(TypeID::P1);
		codeTypes.insert(TypeID::P2);
		codeBlunderDetect.setSysCodeTypes( SatID::systemGPS, codeTypes );

		codeBlunderDetect.setReceiverStatic( false );

		pList.push_back( codeBlunderDetect );
...
//      SolverPPP   pppSolver(isNEU);
		bool addISBEG( employGPS && employGALILEO );
