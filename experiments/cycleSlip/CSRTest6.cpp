...
//
// This program tries to implement PPP using rinex3 with GPS and Galileo system. 
//============================================================================
//
// PPP Positioning
//
...
//	PPP using GPS or Galileo
// Lei Zhao
//
// 2017/02/11
// Add section for specified time interval of processing
// Lei Zhao
//============================================================================
...
#include "MWCSDetector2MGEX.hpp"
   ...
   // Class to resolve integer cycle slips  
...
#include "SolverPPPFB.hpp"
   ...
   // Class to compute the Precise Point Positioning (PPP) solution in
   // forwards-only mode using GNSS combined observation
...
//                       int    precision = 3 );
      ...
      // Method to print solution values
   void printSolution2( ofstream& outfile,
                       const  SolverLMS& solver,
                       const  CommonTime& time,
                       const  ComputeDOP& cDOP,
                       bool   useNEU,
   ...
   void printModel( ofstream& modelfile,
      ...
      // Method to print model values
   void printModel2( ofstream& modelfile,
                    const gnssRinex& gData,
                    int   precision = 4 );


      // Method to print sat dli values
   void printTimeDiffLI(  ofstream& dlifile,
								  CommonTime& time,
                      ...
                      false)
{
   ...
   // Method to print solution values
void ppp::printSolution2( ofstream& outfile,
                              const SolverLMS& solver,
                              const CommonTime& time,
                              const ComputeDOP& cDOP,
                              bool  useNEU,
                              ...
                              int   precision )
{
   ...
   else
   {
	...
	if( ISBEGFlag )
	{
		outfile << solver.getSolution( TypeID::ISBEG ) << "  ";		// ISBEG	- #8
	}
...
}  // End of method 'ppp::printSolution2()'
                           ...
                           int   precision )
{
         ...
         it++ )
   {
      ...
      for( typeValueMap::const_iterator itObs  = (*it).second.begin();
           ...
           itObs++ )
      {
			const TypeID& type( itObs->first );
			const double& value( itObs->second );
			if( type == TypeID::MWubbena )
			{	
				double cycle( value/0.862 );
				   // Print type names and values
				modelfile << type << " ";
				modelfile << cycle << " ";
			}
			if( type == TypeID::prefitP1 || type == TypeID::prefitP2 || 
				 type == TypeID::prefitL1 || type == TypeID::prefitL2 )
			{
				modelfile << type << " ";
				modelfile << value << " ";
			}
									 ...
									 int   precision )
{
      ...
      // Prepare for printing
   dlifile << fixed << setprecision( precision );

	satTypeValueMap satlimap( csetor.getSatLITTimeDiffMap() );

      // Iterate through the GNSS Data Structure
   for ( satTypeValueMap::const_iterator it = satlimap.begin();
         it!= satlimap.end();
         it++ )
	{
			// print time
	   dlifile << static_cast<YDSTime>(time).year         << "  ";    // Year           #1
		dlifile << static_cast<YDSTime>(time).doy          << "  ";    // DayOfYear      #2
		dlifile << static_cast<YDSTime>(time).sod    << "  ";    // SecondsOfDay   #3
      ...
      for( typeValueMap::const_iterator itObs  = (*it).second.begin();
		...
		try
		{
//			double postp1(csetor.getSatPostfitResidual(sat, TypeID::prefitP1));	
//			double postp2(csetor.getSatPostfitResidual(sat, TypeID::prefitP2));	
//			double postl1(csetor.getSatPostfitResidual(sat, TypeID::prefitL1));	
//			double postl2(csetor.getSatPostfitResidual(sat, TypeID::prefitL2));	
//			double qvv(csetor.getSatPostfitResidual(sat, TypeID::postfitP1));
//			double csl1( csetor.getSatFixedCS(sat, TypeID::prefitL1) );
...
//		dlifile << "PostP1: " <<csetor.getSatPostfitResidual(sat, TypeID::prefitP1) << " ";
//			dlifile << "PostP2: " << csetor.getSatPostfitResidual(sat, TypeID::prefitP2) << " ";
//			dlifile << "PostL1: " << csetor.getSatPostfitResidual(sat, TypeID::prefitL1) << " ";
//			dlifile << "PostL2: " << csetor.getSatPostfitResidual(sat, TypeID::prefitL2) << " ";
//		
//			dlifile << "Qvv: " << csetor.getSatPostfitResidual(sat, TypeID::prefitP1) << " ";
		dlifile << endl;
//
	} // End of ' for ( satTypeValueMap::const_iterator it = satlimap.begin(); ...'
	...
	// Method to print model values
void ppp::printModel2( ofstream& ncsfile,
                           const gnssRinex& gData,
                           int   precision )
{

      // Prepare for printing
   ncsfile << fixed << setprecision( precision );

      // Get epoch out of GDS
   CommonTime time(gData.header.epoch);

   ncsfile << static_cast<YDSTime>(time).year         << "  ";    // Year           #1
   ncsfile << static_cast<YDSTime>(time).doy          << "  ";    // DayOfYear      #2
   ncsfile << static_cast<YDSTime>(time).sod    << "  ";    // SecondsOfDay   #3
         ...
         it++ )
   {
		...
		{
			numCSSats++;
			satStr = satStr + " " + StringUtils::asString(sat);
		}
         // Print epoch

         // Print satellite information (Satellite system and ID number)
//      ncsfile << (*it).first << " ";             // System         #4
                                                   // ID number      #5
...
//			const TypeID& type( itObs->first );
//			const double& value( itObs->second );
//			if( type == TypeID::CSL1 || type == TypeID::CSL2 )
//			{	
//				   // Print type names and values
//				ncsfile << type << " ";
//				ncsfile << value << " ";
...
//
   ...
   catch (FileMissingException& e)
   {
      ...
      exit(-1);
			...
			// Read time interval from the configure file
		bool timeIntervalFlag( confReader.getValueAsBoolean( "processingIntervalFlag" ) );

		CommonTime startTime, endTime;
		vector<double> startTimeVec( 6, 0.0);
		vector<double> endTimeVec( 6, 0.0 );
...
//			vector<double> endTimeVec( 6, 0.0 );
			...
			double tempTime;
			size_t i(0);
			while( ( tempTime = confReader.fetchListValueAsDouble( "StartTime")) )
			{
				cout << " temptime: " << tempTime << endl;
				startTimeVec[i] = tempTime;
				i++;
			}  // End of ' while( ( tempTime =  ... '
			

				// Read the end time
			i=0;
			while( ( tempTime = confReader.fetchListValueAsDouble( "EndTime") ) )
			{
				endTimeVec[i] = tempTime;
				i++;
			}  // End of ' while( ( tempTime =  ... '

//				// Construct CommonTime format
//			CivilTime tempS( startTimeVec[0], startTimeVec[1], startTimeVec[2],
//									startTimeVec[3], startTimeVec[4], startTimeVec[5] );
//
//			cout << tempS.hour << " " << tempS.minute << " " << tempS.second << endl;
//			cout << " startTimeVec[0]: " << startTimeVec[0] << endl;
//			cout << " startTimeVec[3]: " << startTimeVec[3] << endl;
//			startTime = tempS.convertToCommonTime();
//			startTime.setTimeSystem(TimeSystem::GPS);
//
//
//			CivilTime tempE( endTimeVec[0], endTimeVec[1], endTimeVec[2],
//									endTimeVec[3], endTimeVec[4], endTimeVec[5] );
//
//			endTime = tempE.convertToCommonTime();
...
//			// Debug code ^^^ 
		...
		if( satSystemInfo.find("GLONASS") != satSystemInfo.end() )
		{
			...
			cout << "Glonass" << " ";
			employGLONASS = true;
			satSysSet.insert( SatID::systemGlonass );
		}
   ...
   while( rnxit != rnxFileListVec.end() )
   {
      ...
      CommonTime initialTime( roh.firstObs ) ;
			...
			// Reconstruct start time and end time 
		if( timeIntervalFlag )
		{
			CivilTime tempCt(initialTime);
				// Only change the year, month and day parts
				// The other parts are read from conf file
				
				// year
			startTimeVec[0] = tempCt.year;
			endTimeVec[0] = tempCt.year;

				// month
			startTimeVec[1] = tempCt.month;
			endTimeVec[1] = tempCt.month;

				// day
			startTimeVec[2] = tempCt.day;
			endTimeVec[2] = tempCt.day;
...
//         continue;
      ...
      cout << " nominal position: " << nominalPos << endl;
...
//		}
      ...
      ProcessingList pList;
			...
			// Class 'CC2NONCC2' is designed for SatID::systemGPS only.
			// Method 'CopyC1ToP1' is used to handle all kinds of receiver,
			// no matter c1p2 or p1p2 receiver.   
		CC2NONCC2 cc2noncc;
...
//			   DCBDataReader dcbStore;
	...
	//		   }
      ...
      LinearCombinationsMGEX comb;
		...
		if( employGPS )
		{
			linearPrefit.addLinear( SatID::systemGPS, comb.p1Prefit );
			linearPrefit.addLinear( SatID::systemGPS, comb.p2Prefit );
			linearPrefit.addLinear( SatID::systemGPS, comb.l1Prefit );
			linearPrefit.addLinear( SatID::systemGPS, comb.l2Prefit );
			//liComb.addLinear( SatID::systemGPS, liComb.liCombinationGPS );
		}
		...
		if( employGALILEO )
		{
			...
			//liComb.addLinear( SatID::systemGalileo, liComb.liCombinationGalileo );
		}
...
//		Triple offsetARP( roh.antennaOffset );
      ...
      if( useantex )
      {
		...
		// Debug code vvv
//		cout << "getDazi(): " << receiverAntenna.getDazi() << endl;
//		cout << "getZen1(): " << receiverAntenna.getZen1() << endl;
//		cout << "getZen2(): " << receiverAntenna.getZen2() << endl;
//		cout << "getDzen(): " << receiverAntenna.getDzen() << endl;
//		cout << "getNumFreq(): " << receiverAntenna.getNumFreq() << endl;
//		cout << "getAntennaComments(): " << receiverAntenna.getAntennaComments()[1] << endl;
		// Debug code ^^^ 
...
//      }
      ...
      pList.push_back(corr);       // Add to processing list
		...
		// Test code vvv
//		string prnFile = confReader.getValue( "PRN_GPS", "DEFAULT"); 
		// Test code ^^^
...
//		bool reInitializeFlag( confReader.getValueAsBoolean("setReInitialize") );
//		if( reInitializeFlag )
//		{
//			pppSolver.setReInitializeFlag( true );
//			pppSolver.setRestartInterval( confReader.getValueAsDouble("reInitialInterv") );
//		}

//		pppSolver.setReInitializeFlag( false );
//		pppSolver.setRestartInterval( 7200 );
...
//      bool isWN( confReader.getValueAsBoolean( "coordAsWhiteNoise") );
			...
			// Get precision standard
//		double preStandard( confReader.getValueAsDouble( "precisionStandard" ) );
//		pppSolver.setPrecisionStandard( preStandard );
      ...
      if( printmodel )
      {
         ...
         modelName = outputFileName + ".model";
      ...
      {
         //modelName = rnxFile + ".model";
         ncsName = outputFileName + ".numCSSats";
...
//					  >> csEstimator
//					  >> linear1;
//				std::cout << "main_numSats: " << gRin.body.numSats() << std::endl; 
//				gRin  >> markArc;
//				std::cout << "main_numSats after markArk: " << gRin.body.numSats() << std::endl; 
//					  continue;
//
//				gRin.keepOnlySatSystem( SatID::systemGalileo );
//					// Test the SV num
//				size_t SVNum(gRin.body.numSats());
//				if( SVNum < 4 )
//				{
//					cout << "SVNum of Galileo System is <4" << endl;
//					continue;
//				}
//				if( employGPS )
//				{
...
//				break;
         ...
         if ( printmodel )
         {
            ...
            printModel( modelfile,
				...
				// Ask if we are going to print numCSSats 
			if ( printNumCSSats )
         {
            printModel2( ncsfile,
								 gRin,
...
//         if ( printdLI )
      ...
      if ( printmodel )
      {
			cout << "model file is generated: " + modelName << endl;
            // Close model file for this station
         modelfile.close();
      }

         // If we printed the numCSSats, we must close the file
      if ( printNumCSSats )
      {
			cout << "numCSSats file is generated: " + ncsName << endl;
            // Close model file for this station
         ncsfile.close();
      }
